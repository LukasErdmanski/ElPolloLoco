...
async function createLoadRenderSaveImages(imagePaths) {
  const [maxWidth, maxHeight] = getMaxWidthHeightDeviceWindow();
...
/**
 * Gets the maximum width and height of the device window.
 * @function
 * @returns {number[]} An array containing the maximum width and height of the device window.
 */
function getMaxWidthHeightDeviceWindow() {
...
function createLoadRenderSaveImg(path, tempCanvasSettings) {
  return new Promise(async (resolve, reject) => {
    try {
      ...
      const [scaleImg, scaledWidth, scaledHeight] = await createScaleImgWithScaledWidthHeight(
        path,
        maxWidth,
        maxHeight
      );
      // Set up the temporary canvas and context.
      const [tempCanvas, tempCanvasCtx] = setTempCanvasWithContext(
        maxWidth,
        maxHeight,
        tempCanvasesContainer,
        freeTempCanvases
      );
...
/**
 * Creates a promise to create and load an image. The promise resolves when the image has been created and loaded.
 * @function
 * @param {string} imgPath - The path of the image file to be loaded.
 * @returns {Promise<HTMLImageElement>} A promise that resolves when the image has been created and loaded.
 */
 ...
 * @returns {Array.<*>} An array containing the scaled image element and its dimensions.
 *                      Index 0 contains the scaled image element (HTMLImageElement),
 *                      index 1 contains the scaled width (number), and
 *                      index 2 contains the scaled height (number).
 */
 ...
 * @param {Object} soundsObj - The object that contains categorized paths and initial volume of all sounds to be
 * created, loaded and saved.
 * @param {Array.<string>} allSoundPaths - An array of all sound paths.
 * @param {Array.<Array.<string>>} allPathPositionsInSoundsObj - An array of all sound path positions in the 'soundsObj'.
 * @returns {Array.<*>} An array containing the allSoundPaths and allPathPositionsInSoundsObj arrays.
 *                      Index 0 contains an array of all sound paths (Array.<string>), and
 *                      index 1 contains an array of all sound path positions in the 'soundsObj' (Array.<Array.<string>>).
 */
